...
exports.loginUser=catchAsyncErrors(async(req,res,next)=>{
    ...
    // Checking if the user has given both email and password or not
    if(!email || !password){
...
exports.forgotPassword = catchAsyncErrors(async(req,res,next)=>{
    ...
    try {
        ...
        await sendEmail({
            email: user.email,
            subject: `Password Recovery - Praviaa.com`,
            message
        });
        res.status(200).json({
            success: true,
...
exports.resetPassword = catchAsyncErrors(async(req,res,next)=>{
    ...
    if(req.body.password !== req.body.confirmPassword){
        return next(new ErrorHandler("Password doen not matched",400));
    }
